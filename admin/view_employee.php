...
$qry = $db->db->query("SELECT e.*,(e.lastname || ', ' || e.firstname || ' ' || e.middlename) as `name`,d.name as dept, p.name as position FROM `employee_list`e inner join `position_list` p on e.position_id = p.position_id INNER JOIN `department_list` d on e.department_id = d.department_id where e.employee_id = '{$_GET['id']}'")->fetch_assoc();
foreach($qry as $k => $v){
    if(!is_numeric($k))
    $$k = $v;
}

...
if(isset($_GET['id'])){
    $sql = "SELECT a.*,(e.lastname || ', ' || e.firstname || ' ' || e.middlename) as name,e.employee_code,d.name as dept,p.name, a.imagen as pos FROM attendance_list a inner join employee_list e on a.employee_id = e.employee_id inner join department_list d on e.department_id = d.department_id inner join position_list p on e.position_id = p.position_id where attendance_id = '{$_GET['id']}'";
    $qry = $db->db->query($sql)->fetch_assoc();
    if($qry){
        foreach($qry as $k => $v){
            if(!is_numeric($k))
            $$k = $v;
...
<div class="container-fluid">
    <div class="col-12">
        <div class="row">
                ...
                <dl>
                    ...
                    <dd class="ps-4">
                        <p>
                            ...
                            Posición: <b><?php echo $location ?></b><br>
            ...
            <div class="col-md-2">
               <img src="<?php echo "../sistem_aux/fotos_js-master/".$imagen ?>" alt="No requeridos" style="width:150px; heidth:150px;">
            </div>
            <div class="col-md-12">
